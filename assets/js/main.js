...
function continueCalculation() {
 ...
 // UENR grading 
  ...
  // 3.50-4.00         1st Class
  // 3.00-3.49          2nd Class Upper
  // 2.50-2.99         2nd Class Lower
  // 2.00-2.49         3rd Class
  // 1.00-1.99         Pass
  // 0.-0.99           Fail
...
function stopCalculation() {
  let firstClass = averageGPA(years.value, 3.52);
  let content = `
    ...
    years.value
      ...
      <div>
        ${
          firstClass > 3.7
            ? ""
            : `<p>With your score you will need <strong>${firstClass}</strong> to remain at <strong>1st class</strong> for each of the semesters left</p>`
        }

        <p>With current score your will need <strong>${averageGPA(
          years.value,
          2.52
        )}</strong> to remain at  <strong>2nd class upper</strong> for each of the semesters left</p>

        <p>With current score your will need <strong>${averageGPA(
          years.value,
          1.52
        )}</strong> to remain at <strong>2nd class lower</strong> for each of the semesters left</p>
